...
/**
 * Extracts entity properties by removing style properties and 'cas'
 * Keeps 'type' for discriminated union typing and excludes 'geom' (handled separately)
 */
function extractEntityProperties<T extends Record<string, unknown>>(
  properties: T
): Omit<
  T,
  | 'type'
...
> {
  const {
    type: _type,
    expectedEligibilityType: _expectedEligibilityType,
    fill: _fill,
    'fill-opacity': _fillOpacity,
    stroke: _stroke,
    'stroke-opacity': _strokeOpacity,
    'stroke-width': _strokeWidth,
    'marker-color': _markerColor,
    'marker-size': _markerSize,
    'marker-symbol': _markerSymbol,
    geom: _geom,
    ...entityProps
  } = properties;
  return entityProps;
    ...
    switch (feature.properties.type) {
      case 'commune': {
        await kdb
          ...
          .values({
            ...extractEntityProperties(feature.properties),
            geom,
          })
          .execute();
        break;
