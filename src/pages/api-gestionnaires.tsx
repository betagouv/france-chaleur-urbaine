...
import { type DemandDTO, demandeStatutDoc, zDemande } from '@/modules/partner-api/schema';
import { type FlattenKeys, hasProperty } from '@/utils/typescript';
...
const SAMPLE_VALUES: Record<FlattenKeys<DemandDTO, string | number | boolean | null>, unknown> = {
...
const ApiGestionnairesPage = () => (
  <SimplePage
    ...
    description="Récupérez automatiquement vos demandes de raccordement et mettez-les à jour depuis votre CRM."
  >
    <Box py="4w" className="fr-container">
      <Heading as="h1" color="blue-france">
        Connecter votre CRM à France Chaleur Urbaine
      </Heading>
      <Text mb="4w">
        France Chaleur Urbaine met à disposition une API REST permettant aux gestionnaires de réseaux d'interfacer leur CRM avec la
        plateforme : récupérer automatiquement les demandes de raccordement de leurs réseaux et les mettre à jour. Pour activer un accès,
        contactez l'équipe FCU.
      </Text>
      ...
      <Text mb="2w">
        Le champ <code>statut</code> prend l'une des valeurs suivantes (à privilégier dans votre intégration plutôt que le libellé) :
      </Text>
      <TableBasic bordered>
        <thead>
          <tr>
      ...
      <Box className="my-2w overflow-x-auto rounded bg-grey-975 p-3">
        <code>{`curl -X PATCH -H "Authorization: Bearer <token>" -H "Content-Type: application/json" -d '{"statut":"recontacte","commentaire":"RDV technique planifié"}' "https://france-chaleur-urbaine.beta.gouv.fr/api/v2/demands/<id>"`}</code>
      </Box>
