...
/**
 * Carte nationale. Reads the legacy deep-link params from the URL
...
const Carte = () => {
  ...
  const config = useMemo<DeepPartial<MapConfiguration>>(() => {
    // The legacy `regions` filter key has no map equivalent (region framing is handled via coord/zoom);
    // it's harmless if present and is never re-emitted by the URL sync.
    const rdcFilters = ((): DeepPartial<MapConfiguration['reseauxDeChaleur']> & {
      gestionnaires?: string[];
      maitreOuvrage?: string[];
    } => {
      if (!rdcFiltersParam) return {};
      try {
        return JSON.parse(rdcFiltersParam);
      } catch {
        return {};
      }
    })();

    // `gestionnaires` / `maitreOuvrage` are top-level filters, not reseauxDeChaleur dimensions.
    const { gestionnaires, maitreOuvrage, ...rdcDimensions } = rdcFilters;

    const draft: DeepPartial<MapConfiguration> = {
      customGeojson: true,
      ...(gestionnaires?.length ? { filtreGestionnaire: gestionnaires } : {}),
      ...(maitreOuvrage?.length ? { filtreMaitreOuvrage: maitreOuvrage } : {}),
      reseauxDeChaleur: { show: true, ...rdcDimensions },
