...
$psql postgres://postgres:postgres_fcu@$DOCKER_HOST:5432/postgres <<EOF
  ...
  create view opendata.reseaux_de_chaleur as (
    SELECT
      ...
      -- un prix < 1 € signifie « non communiqué » : exporté null, comme un prix absent (seuil : businessRules.heatNetworkPriceCommunicatedMin)
      (case when "PM" >= 1 then "PM" end)::numeric(10, 2) as "PM",
      "PV%"::integer,
      "PF%"::integer,
      (case when "PM_L" >= 1 then "PM_L" end)::numeric(10, 2) as "PM_L",
      (case when "PM_T" >= 1 then "PM_T" end)::numeric(10, 2) as "PM_T",
      "Rend%"::integer,
      "Dev_reseau%"::integer,
  ...
  create view opendata.reseaux_de_chaleur_shp as (
    SELECT
      ...
      -- un prix < 1 € signifie « non communiqué » : exporté vide, comme un prix absent (seuil : businessRules.heatNetworkPriceCommunicatedMin)
      (case when "PM" >= 1 then "PM" end)::numeric(10, 2)::text as "PM",
      "PV%"::integer::text,
      "PF%"::integer::text,
      (case when "PM_L" >= 1 then "PM_L" end)::numeric(10, 2)::text as "PM_L",
      (case when "PM_T" >= 1 then "PM_T" end)::numeric(10, 2)::text as "PM_T",
      "Rend%"::integer::text,
