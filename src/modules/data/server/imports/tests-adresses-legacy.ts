...
export const importTestsAdressesLegacy = defineImportFunc(async ({ logger }) => {
  ...
  logger.info(`Found ${eligibilityTests.length} eligibilityTests`);
  ...
  const testLimit = limitFunction(
    async ([index, { id, result, addresses_count, file }]: [number, (typeof eligibilityTests)[number]]) => {
      logger.info(`Processing eligibilityTest ${index + 1}/${eligibilityTests.length} (${id})`);
      ...
      try {
        ...
        if (file === 'Not available' || !file) {
          logger.error(`No result for eligibilityTest ${id}`);
      ...
      logger.info(`Processing ${addresses.length} addresses / ${addresses_count}`);
      ...
      await kdb.transaction().execute(async (trx) => {
        ...
        const chunkLimit = limitFunction(
          async ([chunkIndex, chunk]: [number, string[]]) => {
            ...
            const chunkResults = await getAddressesCoordinates(lines, logger);
  ...
  logger.info(`Bypassed ${addressesNotValid} addresses`);
  logger.info(`Processed ${processedAddresses} addresses`);
});
