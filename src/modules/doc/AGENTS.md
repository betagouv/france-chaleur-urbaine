...
- **Typography**: inside guillemets, use `&nbsp;` entities — `«&nbsp;texte&nbsp;»` — in prose and tables (not inside Mermaid chart strings). Entities, not literal U+00A0 (editing tools flatten it); `search-extract.ts` normalizes them for the search index.
...
- `client/inventories/*` — tables generated from code registries (email registry `trigger` field, `cron.config.ts`, `DEMANDE_STATUS`, `demandeChaleurRenouvelableStatuses`, `demandeChaleurRenouvelableProjectStates`, events catalog): they cannot drift. `event-groups.ts` holds the events grouping (shared with the coverage test).
