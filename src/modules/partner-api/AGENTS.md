...
- `statut` is the **API value** (`a_traiter`, `recontacte`, …); the DB stores the internal `DEMANDE_STATUS`
  label. Conversion is an **explicit mapping** (`schema.ts:apiStatutByStatus`) decoupled from internal enum naming, so the
  contract doesn't move when the enum is renamed.
