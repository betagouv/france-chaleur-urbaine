...
import { type DemandDTO, statutToLabel, zListDemandsQuery, zPatchDemandInput } from '../schema';
 ...
 */
export const patchDemand: PartnerApiHandler<DemandDTO | null> = async (req, res, auth) => {
  ...
  await kdb
    ...
    .set({
      ...
      ...(input.statut !== undefined && { legacy_values: mergeLegacyValues({ Status: statutToLabel(input.statut) }) }),
      updated_at: new Date(),
    })
    .where('id', '=', id)
    .execute();

  // Traçabilité : pas d'utilisateur (auteur = credential), author_id null + provenance en data.
  await createUserEvent({
    author_id: null,
    context_id: id,
    context_type: 'demand',
    data: { credential_id: auth.credentialId, organization_id: auth.organizationId, source: 'partner_api', ...input },
    type: 'demand_updated',
  });

  const row = await baseDemandQuery().where('demands.id', '=', id).executeTakeFirst();
  return row ? toDemandDTO(row) : null;
};
