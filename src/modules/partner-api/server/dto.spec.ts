...
import { apiStatutByStatus, labelToStatut, statutToLabel, zDemande } from '../schema';
...
describe('toDemandDTO', () => {
  it('maps a demand row to the full partner DTO', () => {
    expect(toDemandDTO(baseRow)).toEqual({
      ...
      eligibilite: { dans_pdp: false, distance_reseau_m: 85, eligible: true },
      id: '11111111-1111-4111-8111-111111111111',
      localisation: {
        adresse: '12 rue des Lilas',
        code_postal: '93200',
      ...
      reseau: { id_fcu: 1234, identifiant_sncu: '7501C', nom: 'Réseau de Saint-Denis', type: 'reseau_de_chaleur' },
      statut: 'recontacte',
    });
  });
  ...
  it('defaults an empty/absent status to the initial value', () => {
    expect(toDemandDTO({ ...baseRow, legacy_values: { ...legacy, Status: '' } }).statut).toBe('a_traiter');
  });
});

describe('statut conversion', () => {
  it.each(Object.entries(apiStatutByStatus))('round-trips %s ↔ %s', (status, statut) => {
    expect(labelToStatut(status)).toBe(statut);
    expect(statutToLabel(statut)).toBe(status);
  });
});
