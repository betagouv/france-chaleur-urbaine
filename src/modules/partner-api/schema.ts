...
// ─── Statut : valeur API ↔ statut interne (DEMANDE_STATUS) ────
// Valeurs API découplées de l'enum : le contrat ne bouge pas si l'enum est renommée. `legacy_values.Status` = libellé interne.
export const apiStatutByStatus = {
  [DEMANDE_STATUS.TO_PROCESS]: 'a_traiter',
  [DEMANDE_STATUS.UNREALISABLE]: 'non_realisable',
  [DEMANDE_STATUS.RECONTACTED]: 'recontacte',
  [DEMANDE_STATUS.COMMERCIAL_PROPOSAL]: 'proposition_commerciale',
  [DEMANDE_STATUS.VOTED]: 'vote',
  [DEMANDE_STATUS.WORK_IN_PROGRESS]: 'travaux_en_cours',
  [DEMANDE_STATUS.DONE]: 'realise',
  [DEMANDE_STATUS.ABANDONNED]: 'abandonne',
} as const satisfies Record<DEMANDE_STATUS, string>;

export type DemandStatut = (typeof apiStatutByStatus)[DEMANDE_STATUS];

// Reverse : valeur API → libellé interne.
const statusByStatut = Object.fromEntries(Object.entries(apiStatutByStatus).map(([status, statut]) => [statut, status])) as Record<
  DemandStatut,
  DEMANDE_STATUS
>;

/** Libellé interne → valeur API ; absent/inconnu ⇒ défaut. */
export const labelToStatut = (label: string | null | undefined): DemandStatut =>
  apiStatutByStatus[label as DEMANDE_STATUS] ?? apiStatutByStatus[demandStatusDefault];

/** Valeur API → libellé interne à écrire dans `legacy_values.Status`. */
export const statutToLabel = (statut: DemandStatut): DEMANDE_STATUS => statusByStatut[statut];

/** Doc publique : valeur API ↔ libellé interne. */
export const demandeStatutDoc = Object.entries(apiStatutByStatus).map(([status, value]) => ({ meaning: status, value }));

export const zDemandeStatut = z
  .enum(Object.values(apiStatutByStatus))
  .describe(`Statut d'avancement : ${demandeStatutDoc.map((s) => `${s.value} (${s.meaning})`).join(', ')}.`);
...
const zEligibilite = z
  .object({
    ...
    eligible: z.boolean().describe('Adresse jugée raccordable au moment de la demande.'),
  })
