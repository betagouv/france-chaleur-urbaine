...
/**
 * Adds a drag/drop + paste overlay on the canvas and pushes the parsed GeoJSON
 * to a map source (and the optional `onDrop` callback).
 */
export function FileDropHandler({ onDrop, sourceId = 'customGeojson' }: FileDropHandlerProps) {
  const map = useMapInstance();
  const [dragging, setDragging] = useState(false);
  // Map viewport padding = the area not covered by the legend drawer (reflected
  // by `LegendPaddingSync`). Inset the dropzone by it so it stays fully visible.
  const [padding, setPadding] = useState<maplibregl.PaddingOptions>({ bottom: 0, left: 0, right: 0, top: 0 });
  ...
  useEffect(() => {
    ...
    const onDragOver = (event: DragEvent) => {
      ...
      setDragging(true);
      const next = map.getPadding();
      setPadding((prev) =>
        prev.top === next.top && prev.right === next.right && prev.bottom === next.bottom && prev.left === next.left ? prev : next
      );
    };
  ...
  return (
    <div
      ...
      className={`pointer-events-none absolute m-12 grid place-content-center border-4 border-dashed border-black bg-black/30 text-xl font-bold transition-opacity duration-300 ${dragging ? 'opacity-100' : 'opacity-0'}`}
      style={{ bottom: padding.bottom, left: padding.left, right: padding.right, top: padding.top }}
    >
