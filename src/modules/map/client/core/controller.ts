  ...
  return {
    fitBounds: (bbox, options) => {
      map.fitBounds(
        ...
        // Omit `maxZoom` when undefined: MapLibre's `extend` would copy it over its own default and
        // compute a NaN zoom (`Math.min(zoom, undefined)`), crashing on an `Invalid LngLat (NaN)`.
        {
