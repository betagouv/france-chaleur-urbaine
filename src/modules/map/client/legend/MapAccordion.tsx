 ...
 *
 * Expansion is either URL-persisted (`urlStateId` → `?accordions=` on `/carte`,
 * local-state fallback elsewhere) or controlled (`expanded` / `onExpandedChange`),
 * made mutually exclusive at the type level — see {@link ExpansionProps}.
 */
  ...
  return (
    <AccordionRoot defaultExpanded={rest.defaultExpanded} expanded={expanded} setExpanded={setExpanded} className={className}>
      {/* Row container is `items-stretch` so the chevron toggle on the right
          can span the full row height (DSFR `.fr-accordion__btn`
      ...
      {/* Default padding mirrors the per-legend wrapper (`flex flex-col pt-2
