...
> The interactive map module. This is the live architecture reference.

## Status

✅ This module renders every map across the app. Dev sandbox: `/dev/map`.
...
- High-fidelity click/hover (15px buffer, geometry priority, turf snap).
...
| `legend` | `false` | `false` / `'hidden'` (button only) / `'auto'` (open on ≥ 768px). When enabled, `MapLegend` owns the `?tabId` URL param (nuqs) — host pages must not reuse `tabId` for their own state. |
...
src/modules/map/
  ...
  client/
    ...
    interactions/
      ...
      MapInteractions.tsx     # single click/hover authority: turf snap + popup portal + dispatch to clickHandlers (paused while isDrawing)
      clickHandlers.ts        # 2 click primitives — useMapFeatureClick (base listener) + useMapClickCapture (pause base, like drawing). See "Click handling" below
      ...
      MapFitBounds.tsx        # <MapFitBounds> — declarative fitBounds (mapReady-gated, essential, value-based deps)
      MapFlyTo.tsx            # <MapFlyTo> — declarative flyTo (same conventions)
    ...
    layers/
      ConfiguredLayers.tsx    # component + useConfiguredLayers hook (the config-driven static layer set)
      all-layers.ts           # the built-in layer set mounted by <Map>
      useMapLayers.ts         # useMapLayers hook + <MapLayers> — data-driven ad-hoc sources/layers for a single component (see "Ad-hoc dynamic layers" below)
      AdressesEligiblesLayer.tsx # drives the built-in `adressesEligibles` source via useMapLayers (+ autoFit / flyToLocation / onSelect) — demand pages + ProEligibilityTestItem
    ...
    legend/
      ...
      ReseauxDeChaleurFilters.tsx    # filter panel bound to the Jotai config store
    search/
      ...
      legacy/
        CardSearchDetails.tsx # ⚠️ styled-components — to port/revisit (eligibility result card)
    dev/
      Sandbox.tsx             # one <Map> piloted by a mini-form (/dev/map)
      FlyToButtons.tsx
```
...
| Export | Description |
|---|---|
| `MapSourceLayersSpecification` | Source + sublayers + `isVisible(config)` / `filter(config)` / optional `popup`. |
| `MapLayerSpecification` | One render layer with popup handler. |
| `reseauxDeChaleurLayersSpec` | Réseaux de chaleur spec (`layers/specs/reseauxDeChaleur.tsx`). |
...
For sources/layers driven by **runtime data** (not the static config-driven set), use **`useMapLayers` / `<MapLayers>`** (`layers/useMapLayers.ts`) — data-driven, mounted as a child of `<Map>`:

```tsx
<Map config={…}>
  <MapLayers
    sources={[{ id: 'user-territories', data: featureCollection }]}  // new geojson source, or push data into an existing one
    layers={[{ id: 'highlight', type: 'line', source: 'reseaux-de-chaleur', filter, paint }]}  // on a base or own source
  />
</Map>
```

It centralizes the error-prone lifecycle (`mapReady` gating, source-before-layer ordering, `userResources` tracking so overlays survive a style swap, teardown order, vector `source-layer: 'layer'` default). Pass **stable/memoized** `data` references — data is synced via `setData` on reference change. If a source `id` already exists (a static spec source like `adressesEligibles` / `geomUpdate`), it is left in place and only its data is updated. Single-component ownership only — no cross-component effect ordering.

- **Static, config-toggled layers** (50+ app layers) → spec in `layers/specs/` + `all-layers.ts` (the `ConfiguredLayers` system). Not `useMapLayers`.
- **Camera moves** (`fitBounds` / `flyTo`) → declarative `<MapFitBounds bbox … />` / `<MapFlyTo center zoom … />` (`interactions/MapFitBounds.tsx`, `MapFlyTo.tsx`), children of `<Map>`. They gate on `mapReady`, default to `essential: true` (survive reduced-motion), use value-based deps, and react to target changes only (the **initial** camera position is `initialView`'s job). `useMapLayers` is layers/sources only.
- **Click handling** — `MapInteractions` owns the base click/hover/popup (one query, one resolved feature). Two primitives (`interactions/clickHandlers.ts`) — keep it to these two, no per-layer priority system:
  - **`useMapFeatureClick(source, cb)`** — a passive listener on a **base** interactive layer's click (its popup still opens). E.g. `AdressesEligiblesLayer` selecting a demand.
  - **`useMapClickCapture(active)`** — while `active`, **pause** base interactions (no popup/hover), exactly like the drawing tools' mode. For a tool that *owns* the click: it keeps its **own** layer-scoped `map.on('click', layerId, …)` and calls this so nothing else reacts meanwhile. E.g. `BdnbBatimentSelector` (`useMapClickCapture(value === null)`). This is the way to "say no to other interactions while clicking."
- **Hover** for a bespoke tool stays imperative via `useMapInstance()` + `useMapReady()`.

Deliberately a data-driven hook, **not** JSX `<MapSource>`/`<MapLayer>` primitives: avoids fighting React's child-first effect ordering, smaller public surface, validated against 3 real consumers (demand pages, `AdminReseauxPage` geomUpdate, `PermissionsMap`).
