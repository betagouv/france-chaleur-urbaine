...
- **Refusal follow-up link**: `originDemandId` comes from the “Raccordement non réalisable” email URL. Its presence suppresses the heat-network solution and bypasses classic demand creation; it is validated against an existing non-deleted `demands` row with status `Non réalisable` and the same email before being stored as `demands_chaleur_renouvelable.origin_demand_id`.
