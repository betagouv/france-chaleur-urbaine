...
- **Scrolling**: by default the table grows with the page and virtualization follows the window scroll (no height to choose; the table must fit its container width, `table-layout: fixed` squeezes unsized columns). Pass `height` for a bounded scroll container with horizontal scroll (wide admin tables, table beside a map).
