...
import React, { type CSSProperties, type DetailedHTMLProps, forwardRef, type ImgHTMLAttributes, memo, type ReactNode } from 'react';
      ...
      <div
        ...
        className={cx(
          fr.cx(
            ...
            horizontal && ratio !== undefined && `fr-card--horizontal-${ratio === '33/66' ? 'tier' : 'half'}`,
            (() => {
              switch (size) {
                case 'large':
                  return 'fr-card--lg';
      ...
      >
        <div className={cx(fr.cx('fr-card__body'), classes.body)}>
          <div className={cx(fr.cx('fr-card__content'), classes.content)}>
            ...
            {desc !== undefined && <p className={cx(fr.cx('fr-card__desc'), classes.desc)}>{desc}</p>}
            <div className={cx(fr.cx('fr-card__start'), classes.start)}>
              {start}
              {detail !== undefined && <p className={cx(fr.cx('fr-card__detail'), classes.detail)}>{detail}</p>}
            </div>
            <div className={cx(fr.cx('fr-card__end'), classes.end)}>
              {end}
              {endDetail !== undefined && <p className={cx(fr.cx('fr-card__detail'), classes.endDetail)}>{endDetail}</p>}
            </div>
          </div>
          {footer !== undefined && <div className={cx(fr.cx('fr-card__footer'), classes.footer)}>{footer}</div>}
        </div>
        ...
        {imageComponent !== undefined && (
          <div className={cx(fr.cx('fr-card__header'), classes.header)}>
            <div className={cx(fr.cx('fr-card__img'), classes.img)}>{imageComponent}</div>
            {badge !== undefined && (
              <ul className={cx(fr.cx('fr-badges-group'), classes.badge)}>
                <li>{badge}</li>
              </ul>
