...
export const Footer = memo(
  forwardRef<HTMLDivElement, FooterProps>((props, ref) => {
    ...
    return (
      <div id={rootId} className={cx(fr.cx('fr-footer'), classes.root, className)} role="contentinfo" ref={ref} style={style} {...rest}>
        {linkList !== undefined && (
            ...
            <div className={fr.cx('fr-container')}>
              ...
              >
                {linkList.map(
                  (column, columnIndex) =>
                    column !== undefined && (
                      <div key={`fr-footer__top-cat-${columnIndex}`} className={fr.cx('fr-col-12', 'fr-col-sm-3', 'fr-col-md-2')}>
                        {column?.categoryName && <h3 className={fr.cx('fr-footer__top-cat')}>{column?.categoryName}</h3>}
        ...
        <div className={fr.cx('fr-container')}>
          <div className={cx(fr.cx('fr-footer__body'), classes.body)}>
            <div className={cx(fr.cx('fr-footer__brand', 'fr-enlarge-link'), classes.brand)}>
              ...
              {operatorLogo !== undefined && (
                <Link {...homeLinkProps} className={cx(fr.cx('fr-footer__brand-link'), classes.brandLink, homeLinkProps.className)}>
                  <img
                ...
                {domains.map((domain, i) => (
                  <li className={cx(fr.cx('fr-footer__content-item' as any), classes.contentItem)} key={i}>
                    <a
                      ...
                      id={`footer-${domain.replace(/[@]/g, '').replace(/[\/]/g, '-').replace(/\s/g, '-')}-link`}
                    >
                      {domain}
                    </a>
                  </li>
          ...
          {partnersLogos !== undefined && (
            <div className={cx(fr.cx('fr-footer__partners'), classes.partners)}>
              <h2 className={cx(fr.cx('fr-footer__partners-title'), classes.partnersTitle)}>{t('our partners')}</h2>
                ...
                {mainPartnersLogo !== undefined && (
                  <div className={cx(fr.cx('fr-footer__partners-main'), classes.partnersMain)}>
                    {(() => {
                      ...
                      const hasLinkProps = mainPartnersLogo.linkProps !== undefined || mainPartnersLogo.href !== undefined;
            ...
            <ul className={cx(fr.cx('fr-footer__bottom-list'), classes.bottomList)}>
              {[
                ...
                <li className={cx(fr.cx('fr-footer__bottom-item'), classes.bottomItem, className)} key={i}>
                  {!typeGuard<FooterProps.BottomItem>(bottomItem, bottomItem instanceof Object && 'text' in bottomItem) ? (
                    bottomItem
                  ) : (
                    <FooterBottomItem
                      classes={{
  ...
  frMessages: {
    ...
    'license mention': (p: { licenseUrl: string }) => (
      <>
        ...
        <a href={p.licenseUrl} target="_blank" title="licence etalab-2.0 - ouvre une nouvelle fenêtre" id="footer-etalab-licence-link">
          licence etalab-2.0
        </a>
      </>
    ),
...
export function FooterBottomItem(props: FooterBottomItemProps): JSX.Element {
  ...
  const className = cx(
    fr.cx('fr-footer__bottom-link', ...(bottomItem.iconId !== undefined ? ([bottomItem.iconId, 'fr-link--icon-left'] as const) : [])),
    classes.bottomLink,
    classes.root,
    className_props
  );
