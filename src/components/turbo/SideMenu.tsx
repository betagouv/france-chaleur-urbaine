    ...
    return (
      ...
      >
        <div className={cx(fr.cx('fr-sidemenu__inner'), classes.inner)}>
          <button hidden aria-expanded="false" aria-controls={collapseId} className={cx(fr.cx('fr-sidemenu__btn'), classes.button)}>
            {burgerMenuButtonText}
          </button>
          <div className={fr.cx('fr-collapse')} id={collapseId}>
            {title !== undefined && (
              <div className={cx(fr.cx('fr-sidemenu__title'), classes.title)} id={titleId}>
                {title}
              </div>
            )}
            <ul className={cx(fr.cx('fr-sidemenu__list'), classes.list)}>
              {items.map((item, i) => {
                const getItemRec = (params: { item: SideMenuProps.Item; key: string; level: number }) => {
                  ...
                  return (
                    <li key={key} className={cx(fr.cx('fr-sidemenu__item'), classes.item)}>
                      {'items' in item ? (
                        <>
                          {(() => {
                            const ComponentToUse = item.linkProps !== undefined ? Link : 'button';

                            return (
                              // @ts-expect-error
                              <ComponentToUse
                                aria-expanded={(item.expandedByDefault ?? false) ? 'true' : 'false'}
                          ...
                          <div className={fr.cx('fr-collapse')} id={itemId}>
                            <ul className={cx(fr.cx('fr-sidemenu__list'), classes.list)}>
                        ...
                        <Link
                          ...
                          className={cx(fr.cx('fr-sidemenu__link'), classes.link, item.linkProps?.className)}
                        >
                          {item.text}
                        </Link>
                      )}
