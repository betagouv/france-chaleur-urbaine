...
export type TileProps = {
  ...
  classes?: Partial<
    Record<
      'root' | 'content' | 'title' | 'header' | 'link' | 'button' | 'body' | 'desc' | 'detail' | 'start' | 'img' | 'imgTag' | 'artwork',
      string
    >
  >;
    ...
    return (
      <div
        ...
        className={cx(
          fr.cx(
            'fr-tile',
            enlargeLinkOrButton && (linkProps ? 'fr-enlarge-link' : buttonProps ? 'fr-enlarge-button' : null),
          ...
          <div className={cx(fr.cx('fr-tile__content'), classes.content)}>
            <HtmlTitleTag className={cx(fr.cx('fr-tile__title'), classes.title)}>
              {linkProps !== undefined ? (
                ...
                <button {...buttonProps} className={cx(classes.button, buttonProps.className)} disabled={disabled}>
                  {title}
                </button>
              ) : (
                title
            ...
            {desc !== undefined && <div className={cx(fr.cx('fr-tile__desc'), classes.desc)}>{desc}</div>}
            {detail !== undefined && <div className={cx(fr.cx('fr-tile__detail'), classes.detail)}>{detail}</div>}
            {start !== undefined && <div className={cx(fr.cx('fr-tile__start'), classes.start)}>{start}</div>}
          </div>
        </div>
              ...
              <div className={cx(fr.cx('fr-tile__pictogram'), classes.img)}>
                ...
                >
                  {(['artwork-decorative', 'artwork-minor', 'artwork-major'] as const).map((label) => (
                    <use key={label} className={fr.cx(`fr-${label}`)} xlinkHref={`${imageUrl}#${label}`} />
                  ))}
                </svg>
              </div>
            ) : (
