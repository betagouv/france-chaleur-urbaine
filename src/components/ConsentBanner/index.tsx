...
type FinalityDescription = Parameters<
  typeof createConsentManagement
>[0]['finalityDescription'];
...
const hotjarEnabled =
  clientConfig.tracking.hotjarId && clientConfig.tracking.hotjarSv;
  ...
  return (
    <>
      ...
      {!iframedPaths.some((path) => router.pathname.match(path)) && (
        <>
          {googleEnabled && finalityConsent?.google_analytics && (
            <GoogleTagsScript tagIds={clientConfig.tracking.googleTagIds} />
          )}
          {linkedinEnabled && finalityConsent?.linkedin_insights && (
            <LinkedInScript
              partnerId={clientConfig.tracking.linkInPartnerId as string}
            />
          )}
          {facebookEnabled && finalityConsent?.facebook_pixel && (
            <FacebookScript
              pixelId={clientConfig.tracking.facebookPixelId as string}
            />
          )}
          {hotjarEnabled && finalityConsent?.hotjar && (
            <HotjarScript
              hjid={clientConfig.tracking.hotjarId as string}
              hjsv={clientConfig.tracking.hotjarSv as string}
            />
          )}
        </>
      )}
