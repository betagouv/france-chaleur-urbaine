  ...
  const bodyPopup = `
    ...
    ${`
        <section>
          ${
            consommation
              ? `<strong>${writeTypeConso(
                  consommation.code_grand_secteur
                )}</strong><br />`
              : energy
          ...
          ${
            energy
              ? `Nombre de lot : ${energy.nb_lot_habitation_bureau_commerce}<br />`
              : ''
          }
          ${
            energy
              ? `Chauffage actuel :  ${formatBddText(
                  energy?.energie_utilisee
                )}<br />`
              : ''
          }
          ${
            consommation
              ? `Consommations :  ${consommation?.conso?.toFixed(2)} MW/h<br />`
              : ''
          }
          ${
            energy
              ? `Période de construction : ${formatBddText(
                  energy?.periode_construction
                )}<br />`
              : ''
          }
        </section>
