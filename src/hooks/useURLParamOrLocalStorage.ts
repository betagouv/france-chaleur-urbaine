...
export default function useURLParamOrLocalStorage<T>(
  ...
  const [paramValue, internalSetParamValue] = useState<T | null>(initialURLValue ? parseValue(initialURLValue) : null);

  const setParamValue = useCallback((value: T) => {
    internalSetParamValue(value);
    window.localStorage.setItem(`${LOCAL_STORAGE_KEY_PREFIX}${localStorageKey}`, JSON.stringify(value));
  }, []);

  // set the initial value (after a possible null) and synchronize URL and local storage values
  useEffect(() => {
    const localStorageValue = localStorage.getItem(`${LOCAL_STORAGE_KEY_PREFIX}${localStorageKey}`);
